...
| 会員 | 会員情報が登録済みの顧客（本アプリでは会員登録機能自体は未確定。ASM-001, ISS-022参照） |
| レジ担当者 | 担当者ID・パスワードでログインし、レジ業務を行う店員 |
| 商品マスタ | 商品コード・商品名・単価等を保持するデータ（登録・編集機能の要否は未確定。ISS-004参照） |
...
決済代行サービス・レシートプリンタ連携等の外部インターフェースについては、該当機能自体がスコープ対象範囲外・未確定（ISS-003）であるため、本節には記載しない。
...
| UC-03 | 商品を登録する（手入力） | 商品コードを手入力し購入リストに反映する | 同上 | UC-02と同様の結果 | FR-005, SCR-008 | «include» 商品マスタと照合する。確定操作（Enter／ボタン）は `[要確認 ISS-029]` |
...
| UC-07 | 会員IDを読み込む | 会員IDをスキャン等で読み込む | 購入が未確定 | 会員IDが取引に紐づく。存在しない会員IDの場合の挙動は `[要確認 ISS-008]` | FR-013, FR-014, FR-015, SCR-012 | 購入確定までいつでも実行可（Lv2で緩和、FR-013） |
| UC-08 | 値引きを適用する | 会員ID入力済み・対象期間内の商品に値引きを自動適用する | 会員IDが入力済み。値引き対象商品・期間が設定されている | 値引き額を計算し購入リスト上に表示 | BR-002, FR-011, FR-012, SCR-006 | «extend» UC-02, UC-03（拡張条件：登録商品が値引き対象かつ会員ID入力済み）。`[要確認 ISS-006]` 会員ID入力前スキャン分の扱い、`[要確認 ISS-007]` 複数値引き併用可否 |
| UC-09 | 購入を確定する | 購入リストの内容を確定し取引として記録する | 購入リストに1件以上の商品。ログイン済み | 取引履歴を保存（NFR-001）。税込・税抜合計を提示（FR-017）。確定後の画面遷移・クリア挙動は `[要確認 ISS-025]` | BR-004, FR-016, FR-017, FR-021, SCR-007, SCR-010, DR-005 | «include» 取引履歴を保存する |
...
|---|---|---|---|---|---|
| F-01 | 認証機能 | 担当者ID・パスワードによるログイン認証。担当者情報を画面表示し以降の取引に紐づける | BR-001, FR-019, SCR-001, SCR-011 | DR-007, IF-001 | ログアウト機能の要否は `[要確認 ISS-010]` |
...
| F-03 | 商品登録機能（手入力） | 商品コードを手入力し、商品マスタと照合の上、購入リストへ反映する | FR-005, SCR-008 | DR-001 | 入力確定操作（Enterキー／ボタン等）は `[要確認 ISS-029]` |
| F-04 | 購入リスト管理機能 | 購入リストの一覧表示、商品の選択・数量変更・削除、小計／合計金額の再計算 | FR-006〜010, SCR-003, SCR-004, SCR-005, SCR-009 | DR-002 | |
| F-05 | 値引き機能 | 会員向けの期間限定値引きを対象商品に適用し、値引き額を表示する | BR-002, FR-011, FR-012, SCR-006 | DR-003 | 値引き対象・期間・内容の変更手段は `[要確認 ISS-005]`。会員ID入力前スキャン分への適用タイミングは `[要確認 ISS-006]`。複数値引きの併用可否は `[要確認 ISS-007]` |
| F-06 | 会員ID読み込み機能 | 会員IDをバーコードスキャン等で読み込み、取引に紐づける | FR-013〜015, SCR-012 | DR-004 | 存在しない会員IDの扱いは `[要確認 ISS-008]`。「お客様ID読み込みボタン」操作要否は `[要確認 ISS-026]` |
| F-07 | 税表示・税率管理機能 | 購入確定時の税込・税抜合計金額の算出・表示。消費税率の可変管理 | FR-017, FR-018, SCR-007 | DR-006 | 税率・値引きの変更手段は `[要確認 ISS-005]`。複数税率対応は `[要確認 ISS-012]`。Lv1内部の税表示記述矛盾は `[要確認 ISS-028]` |
| F-08 | 購入確定・取引記録機能 | 購入内容を確定し、取引履歴（日時・商品・数量・金額・会員・担当者・値引き適用内容）をDBへ永続化する | BR-004, FR-016, FR-021, SCR-010 | DR-005, NFR-001 | 確定後の画面クリア等の遷移挙動は `[要確認 ISS-025]` |
| F-09 | 商品マスタ単価変更履歴記録 | 商品マスタの単価が変更された場合に、変更前後の単価・変更日時を記録する。確定済み取引は変更前単価のまま保持する | BR-003, FR-016 | DR-008 | 商品マスタを変更するための画面・API自体は `[要確認 ISS-004]`（商品マスタ登録・編集機能のスコープ確認）のため、本書では4章（画面設計）・6章（API設計）の対象外とし、5章のテーブル定義のみ用意する |

#### トレーサビリティ確認

- 要件定義書のFR-001〜023、BR-001〜004、SCR-001〜012は、上記F-01〜F-09のいずれかに対応しており、機能ブロックへの割り当て漏れはない。
- 決済・レシート発行・返品・在庫管理・商品マスタ登録編集画面・会員登録機能など、要件定義書で対象範囲外・未確定（ISS-003, ISS-004, ISS-020〜022）とされている事項は、機能ブロックとして起こしていない。9章「未決事項・今後の検討課題」で再掲する。
...
flowchart TD
    ...
    Input -->|手入力| Manual["商品コードを入力欄に入力し確定する<br/>（FR-005, F-03／確定操作は要確認 ISS-029）"]
    ...
    Exists -->|あり| AddQty["数量を+1する（FR-004, FR-009）"]
    Exists -->|なし| NewRow["購入リストに新規行を追加する（FR-004）"]

    AddQty --> Discount{"会員IDが入力済み、かつ<br/>登録商品が値引き対象期間内か"}
    NewRow --> Discount

    Discount -->|該当する| ApplyDiscount["値引き額を計算し表示する<br/>（FR-011, FR-012, F-05／併用可否は要確認 ISS-007）"]
...
**フロー説明**: バーコード認識に失敗した場合（スキャン自体が読み取れない場合）のエラー処理は要求仕様書に明記がなく `[AI提案]`（FR-020）であるため、上図では「商品マスタに一致する商品がない」（FR-023、確定要件）場合のみを分岐として明示し、スキャン失敗自体の扱いは注記に留めている（`[要確認 ISS-009]`）。
...
flowchart TD
    ...
    ShowTotal --> AfterAction["確定後の画面クリア・次取引への遷移<br/>（`要確認 ISS-025`：挙動未確定）"]
    AfterAction --> End(["終了"])
```

**フロー説明**: 購入リストが0件の場合の確定操作拒否、および保存失敗時のエラーハンドリングは要求仕様書に明記がないため `[AI提案]` として設計上補ったフローである。確定後に画面（コード入力欄・購入リスト等）がクリアされるかどうかは、Lv1のREQ-059に類似の記述がある一方Lv2では言及がなく、要件定義書ISS-025として未解決のため、本図でも確定的な遷移先を描かず注記に留めている。
...
決済・レシート発行・返品・在庫管理・商品マスタ登録編集・会員登録などの画面は、対応する機能がスコープ対象範囲外・未確定（ISS-003, ISS-004, ISS-020〜022）であるため、画面一覧に含めない。

購入確定時の金額提示についても、Lv1には「ポップアップ等で表示」（REQ-058）という記述がある一方、Lv2には表示形式の指定がなく、独立画面（ポップアップ）とするかSC-02上のインライン表示とするかは `[要確認 ISS-025, ISS-028]` のため、独立した画面IDを付与せずSC-02内の一要素（4.2節 No.16）として扱う。
...
| 3 | 会員IDスキャン／入力操作 | 操作 | 会員カードのバーコードスキャン等で会員IDを読み込む | FR-013 | 「お客様ID読み込みボタン」操作の要否は `[要確認 ISS-026]` |
| 4 | バーコードスキャンエリア（カメラ） | 操作 | カメラで商品バーコードを読み取る | FR-001, REQ-006 | |
| 5 | 商品コード手入力欄 | 入力 | 商品コードを手入力する | SCR-008, REQ-011, REQ-043 | 入力確定操作（Enterキー／専用ボタン等）は `[要確認 ISS-029]` |
| 6 | 商品名称表示エリア | 表示 | 読み込み・入力された商品の名称を表示 | SCR-004, REQ-045 | Lv1の「一時表示」領域。手入力フローで継続使用されるかは `[要確認 ISS-024]` |
| 7 | 商品単価表示エリア | 表示 | 読み込み・入力された商品の単価を表示 | SCR-004, REQ-046 | 同上 |
| 8 | 購入リストへ追加ボタン | 操作 | 検索結果（6・7）を購入リストへ反映する | REQ-047 | Lv2はバーコードスキャン時に直接リスト反映される記述（FR-001〜004）のため、本ボタンが必要なのは手入力フロー（SC-02 No.5）に限られる可能性がある。2段階UIフローの継続要否は `[要確認 ISS-024]` |
...
| 15 | 合計金額表示（税抜） | 表示 | 税抜きの合計金額を表示 | SCR-007, FR-017 | Lv1内部に「税込みのみポップアップ」という矛盾する記述あり（`[要確認 ISS-028]`）。Lv2の記述（税込・税抜両方）を優先して設計するが未確定 |
| 16 | 購入確定ボタン／確定結果表示 | 操作／表示 | 購入を確定し、税込・税抜合計金額を提示する | SCR-010, FR-017 | 確定結果をポップアップ表示するかインライン表示するかは `[要確認 ISS-025, ISS-028]` |
...
要件定義書のデータ要件（DR-001〜DR-008）をもとに、永続化が必要なエンティティを抽出した。DR-002（購入リストの行データ）については、要求仕様書上「購入確定前の状態を保存しておく」という永続化要件が存在しないため、DBテーブルとしては設計せず、購入確定後にのみ `TRANSACTION_ITEMS` として永続化する前提とした（`[AI提案]`：購入リスト自体はフロントエンド、または確定操作までの一時的なサーバー側状態として保持する）。
...
- `DISCOUNTS` は1レコード＝1対象商品という単純な構造とした。複数商品への一括適用や、値引きの併用可否（ISS-007）は要件定義書上未確定であり、複数商品対応・併用ルールを持たせるかどうかは9章の要確認事項として持ち越す。
- `MEMBERS` のカラムは要件定義書DR-004（DR-004出典REQ-030, REQ-066）に記載された項目（会員ID・氏名・電話番号・住所・性別・年齢）をそのまま反映した。会員登録・編集機能自体はISS-022により対象範囲が未確定のため、このテーブルへのデータ投入経路（画面／バッチ／他システム連携）は本書の設計対象に含めない。
...
| カラム名 | 型 | 制約 | 備考 |
|---|---|---|---|
| product_code | VARCHAR(20) | PK, NOT NULL | 商品コードの規格（JAN／EAN／独自コード等）は `[要確認 ISS-030]` |
...
商品マスタへの新規登録・編集経路は `[要確認 ISS-004]` のため、本テーブルへの書き込みAPI・画面は6章・4章の対象外とする。
...
| カラム名 | 型 | 制約 | 備考 |
|---|---|---|---|
| staff_id | VARCHAR(20) | PK, NOT NULL | |
...
| product_code | VARCHAR(20) | FK → PRODUCTS.product_code, NOT NULL | 1値引き＝1対象商品の単純化。複数商品対応は `[要確認 ISS-005]` |
| discount_type | ENUM('rate','amount') | NOT NULL | 割合値引き・金額値引きの両方に対応（REQ-021） |
| discount_value | DECIMAL(10,2) | NOT NULL | rateの場合は％（例：20％引きは`20.00`）、amountの場合は円。**`[AI提案]` 表記規約の注意**：本テーブルのrateは百分率表記だが、TAX_RATES.rateは小数表記（例：10％は`0.10`）であり、両者の規約は統一していない。実装時に混同しないよう注意（詳細はTAX_RATES節を参照） |
| start_date | DATE | NOT NULL | |
| end_date | DATE | NOT NULL | |

出典：DR-003（REQ-020, REQ-021, REQ-023）。変更手段（コード変更なしに変更・追加できる状態）は `[要確認 ISS-005]`

**`[AI提案]` 複数該当時のタイブレークルール**: 同一`product_code`に対して有効期間（start_date〜end_date）が重複する値引き行を複数登録することを妨げる制約（UNIQUE制約等）は設けていない。複数の値引きが同時に有効な場合にどれを適用するかのルールは要件定義書に記載がなく（ISS-007と関連）、本書でも確定していない。実装時の暫定挙動（例：discount_idが最大＝最も新しく登録されたものを優先する等）を別途定める必要がある。
...
| rate | DECIMAL(5,4) | NOT NULL | 単一税率を前提とした設計。複数税率（軽減税率）対応は `[要確認 ISS-012]`。**`[AI提案]` 表記規約**：小数表記（例：10％は`0.10`）。DISCOUNTS.discount_value（rateの場合は百分率表記）とは規約が異なる点に注意 |
| effective_from | DATE | NOT NULL | |

出典：DR-006（REQ-034, REQ-071）

**`[AI提案]` 「現在有効な税率」の選定ロジック**: 本テーブルは`effective_from`ごとに複数行を持てる構造とした。現在有効な税率は、「`effective_from`が本日以前である行のうち、`effective_from`が最も新しい1件」を採用する、という方式を提案する。この選定ロジック自体は要求仕様書に記載がなく`[AI提案]`である。
...
**備考**: `PurchaseListService` は、購入確定前の購入リスト状態（DR-002相当）を扱うサービスであり、5.1節の判断のとおり対応するDBテーブルを持たない。フロントエンド（Next.js）側の状態管理として実装するか、バックエンド（FastAPI）側にセッション相当の一時状態として持たせるかは、CON-006（役割分担）の具体化事項として `[要確認]`（要件定義書に詳細な役割分担基準の記載なし）とする。
...
| 3 | POST | /api/auth/logout | ログインセッション（リフレッシュトークンの発行記録）を無効化する | F-01 | ― | `[要確認 ISS-010]`：ログアウト機能自体の要否が未確定。実装対象かどうかは要確認事項の解消後に決定 |
...
| 5 | GET | /api/members/{member_id} | 会員IDから会員情報を照会し、存在確認を行う | F-06 | FR-014, FR-032 | 存在しない場合の応答形式は `[要確認 ISS-008]`（挙動そのものが未確定のため） |
| 6 | GET | /api/tax-rates/current | 現在有効な消費税率を取得する | F-07 | FR-018, DR-006 | エンドポイント形態（専用API化）は `[AI提案]`。複数税率対応（ISS-012）が確定した場合は入出力を見直す必要がある |
| 7 | POST | /api/transactions | 購入リストの内容・会員ID・担当者情報をもとに購入を確定し、サーバー側で金額を再計算・照合した上で取引記録（TRANSACTIONS／TRANSACTION_ITEMS）をDBへ保存する（7.4.2(3)節：金額二重検証） | F-08 | FR-016, FR-017, FR-021, NFR-001, DR-005, DR-008 | 税込・税抜合計金額をレスポンスとして返す想定（FR-017） |
...
上記No.1〜3（認証系）はいずれもJWTの発行・検証・失効に関わるエンドポイントであり、詳細な設計思想は7.4.1節を参照。No.7（購入確定）の金額二重検証の詳細設計は7.4.2(3)節を参照。No.9（セッション情報取得）の設計思想は7.4.1節「セッション情報の取得」を参照。
...
| 項目名 | 下限 | 上限 | 根拠・備考 |
|---|---|---|---|
| 購入数量（1商品あたり） | 1 | 99 | FR-007, FR-008, REQ-017, REQ-018（確定要件） |
| 担当者ID文字数 | 4文字 | 20文字 | `[AI提案]` 要求仕様書に文字数規定なし。半角英数字を想定した暫定値 |
| パスワード文字数 | 8文字 | 64文字 | `[AI提案]` NFR-SEC-001「最低限の保護」を具体化する提案値。講義で扱った推奨最小長（8文字以上）に基づく |
| パスワード文字種 | 英字・数字の両方を含む | ― | `[AI提案]` 要求仕様書に記載なし |
| 会員ID文字数 | 4文字 | 20文字 | `[AI提案]` `[要確認]` 要求仕様書に規格の記載なし |
| 商品コード桁数 | 8桁 | 13桁 | `[AI提案]` `[要確認 ISS-030]` JAN／EANコードの一般的な桁数を暫定的に想定。規格確定後に見直しが必要 |
| 値引き率（discount_type=rate） | 0％ | 100％ | `[AI提案]` DISCOUNTS.discount_valueのバリデーション。REQ-021 |
| 値引き金額（discount_type=amount） | 0円 | 対象商品の単価未満 | `[AI提案]` 値引き後金額がマイナスにならないための制約 |
| 消費税率 | 0％ | 20％ | `[AI提案]` 現実的な運用範囲を想定した暫定上限。複数税率対応（ISS-012）確定時に見直し |
| 取引合計金額（税込／税抜） | 0円 | 99,999,999円 | `[AI提案]` DBカラム型（INT）の実務上限に基づく暫定値。DR-005 |
| JWTアクセストークン有効期限 | ― | 30分 | `[AI提案]` 7.4.1節参照 |
| JWTリフレッシュトークン有効期限 | ― | 8時間 | `[AI提案]` 7.4.1節参照（レジ1シフトを想定） |
...
| 分類 | 定義 | 例 | 想定HTTPステータス |
|---|---|---|---|
| バリデーションエラー | リクエストの形式・値が6.2節の設定値や必須項目の制約を満たさない、クライアント起因の入力不正 | 数量が範囲外（FR-008）、必須パラメータ欠落、文字数超過 | 400 Bad Request |
| 業務エラー | リクエスト形式は正しいが、業務ルール上処理を継続できない状態 | 商品マスタ未登録（FR-023）、会員ID不存在（`[要確認 ISS-008]`）、認証失敗（REQ-027）、金額不一致（7.4.2(3)） | 401（認証失敗）／404（未登録）／409（金額不一致等の競合） |
...
| 業務エラー | 画面上部またはポップアップ／トースト通知でエラーメッセージを表示する（例：「商品がマスタ未登録です」FR-023、「金額が一致しませんでした。もう一度お試しください」7.4.2(3)） |
| システムエラー | 汎用エラーメッセージと再試行操作を提示する。内部詳細は表示しない |

購入確定時の税表示形式（ポップアップかインラインか）自体は4.2.2節のとおり `[要確認 ISS-025, ISS-028]` であり、本節のエラー表示方法とは独立した論点として扱う。
...
担当者ID・パスワードによるログイン認証（7.4.1節）。認証不要（未ログイン状態で呼び出す）。

**入力パラメータ（リクエストボディ）**

| パラメータ名 | 型（TS／Pydantic） | 必須/任意 | 制約 |
|---|---|---|---|
| staff_id | `string` ／ `str` | 必須 | 4〜20文字（6.2節） |
| password | `string` ／ `str` | 必須 | 8〜64文字（6.2節） |
...
`[AI提案]`：7.4.1節のJWT設計に伴い新設。認証不要（リフレッシュトークン自体が認証情報）。
...
| 401 | `REFRESH_TOKEN_INVALID` | 期限切れ、署名不正、またはDB上の発行記録が失効済み（ログアウト済み等） |
| 500 | `SYSTEM_ERROR` | 予期しないエラー |

#### 6.4.3 POST /api/auth/logout

`[要確認 ISS-010]`：機能自体の要否が未確定。認証必要（Bearer）。
...
| パラメータ名 | 型（TS／Pydantic） | 必須/任意 | 制約 |
|---|---|---|---|
| product_code | `string` ／ `str` | 必須 | 8〜13桁（6.2節、`[要確認 ISS-030]`） |
...
| パラメータ名 | 型（TS／Pydantic） | 必須/任意 | 制約 |
|---|---|---|---|
| member_id | `string` ／ `str` | 必須 | 4〜20文字（6.2節、`[要確認]`） |
...
| 401 | `AUTH_TOKEN_INVALID` | 未認証 |
| 404 | `MEMBER_NOT_FOUND` | 会員IDに対応する会員が存在しない（`[要確認 ISS-008]`：挙動そのものが未確定） |
| 500 | `SYSTEM_ERROR` | 予期しないエラー |

#### 6.4.6 GET /api/tax-rates/current

現行消費税率の取得（FR-018）。認証必要（Bearer）。
...
購入確定（3.2.2節、7.4.2(3)節の金額二重検証）。認証必要（Bearer）。
...
| 404 | `MEMBER_NOT_FOUND` | member_idを指定したが会員が存在しない（`[要確認 ISS-008]`） |
| 409 | `AMOUNT_MISMATCH` | サーバー再計算値とクライアント参考値が一致しない（7.4.2(3)節） |
...
以下は、要件定義書上「対象範囲外・未確定」（種別C）とされている機能に対応するAPIであり、本章では設計しない。
...
| 在庫管理API | ISS-020 |
| 値引き・消費税率の設定変更API（FR-012, FR-018で「コード変更なしに変更できる」ことは要件化されているが、変更のための手段〈管理画面／設定ファイル／DB直接操作等〉は`[要確認 ISS-005]`） | ISS-005 |
...
要件定義書11章（非機能要件）は、5分類（総称・性能・可用性・セキュリティ・運用保守・移行）のうち、記載があるのは総称（NFR-001）と性能（NFR-PERF-001）の一部、セキュリティ（NFR-SEC-001、`[AI提案]`）のみであり、可用性・運用保守・移行は要求仕様書に一切記載がない（ISS-015, ISS-017, ISS-018）。本章では、記載がある項目は要件定義書の内容に沿って方針を示し、記載がない項目は「未定」であることを明記するに留め、要件定義書にない基準を新たに作り出さない。
...
**Authentication（認証）とAuthorization（認可）の区別**: 本設計では両者を明確に分離する。認証（Authentication）は「ログインした担当者が誰であるかを確認する処理」であり、7.4.1で扱うJWT検証がこれにあたる。認可（Authorization）は「認証済みの担当者が特定の操作を行ってよいかを判断する処理」である。要件定義書には担当者の権限区分（例：一般担当者／管理者）に関する記載が一切なく、本POSアプリはレジ担当者という単一ロールのみを前提としている（BR-001, FR-019）。そのため本書における認可設計は、**「有効なJWTアクセストークンを保持していること＝全レジ機能の利用を認可されている」という単純な二値モデル**に留め、担当者ごとの権限粒度（例：値引き承認権限の限定等）を持たせる設計は行わない。権限区分の要否は `[要確認]`（新規論点、9章に追記）とする。
...
有効期限の具体的な数値は要件定義書に根拠がなく、いずれも `[AI提案]` の暫定値である。性能要件同様、数値目標の妥当性は9章の検討課題として扱う。リフレッシュトークンをDB側でも記録管理する理由は、JWT単体は発行後にサーバー側で強制的に無効化できない（ステートレスな性質上、有効期限が来るまで有効であり続ける）ため、ログアウト機能（`[要確認 ISS-010]`）を実装する場合に「特定のリフレッシュトークンだけを失効させる」操作を可能にするためである。この設計を採用する場合、5章のデータ設計に担当者のリフレッシュトークン発行記録を保持するテーブル（例：REFRESH_TOKENS）の追加が必要になる。本書は6章・7章の拡張を目的としているため当該テーブルの追加は行わず、9章に検討課題として記録するに留める。
...
1. ログイン時、FastAPIは担当者ID・パスワードを検証した後、アクセストークン（claims: `sub`=担当者ID、`iat`、`exp`、`token_type`=`access`）とリフレッシュトークン（`token_type`=`refresh`、DBに発行記録を保存）を発行する。
...
4. FastAPI側は各APIリクエストの受信時、共通の認証ミドルウェア（またはFastAPIの`Depends`による依存性注入）でトークンの署名・有効期限を検証する。検証に成功した場合のみ、リクエストの担当者ID（`sub`クレーム）を後続処理（取引記録への担当者紐づけ等、BR-004・DR-007）で利用する。
...
sequenceDiagram
    ...
    alt 認証成功
        API->>API: アクセストークン発行（exp 30分、sub=担当者ID）
...
2. FastAPIは、リクエストに含まれる商品コードごとに商品マスタ（PRODUCTS）・値引きマスタ（DISCOUNTS）・税率マスタ（TAX_RATES）を参照し、単価・値引き額・税込税抜合計を**サーバー側で独自に再計算**する。フロントエンドから送信された単価・金額そのものは一切信頼せず、計算の入力には用いない（信頼するのは商品コード・数量・会員IDなどの「事実」のみ）。
...
**値引き再計算の適用範囲についての補足（レビュー指摘事項への対応）**: 上記2.の「サーバー側で独自に再計算する」は、3.2.1節のアクティビティ図で確定した各明細行の値引き適用結果（＝商品登録時点で会員IDが入力済みだった行にのみ値引きが付いている状態）を前提に、その内容が正しいかを検算するものであり、**確定時点の最終的な会員ID状態で全明細行の値引き適用を遡って再判定するものではない**（`[AI提案]`）。会員ID入力前にスキャンした商品への遡及適用を行うかどうかはISS-006そのものであり、ISS-006が「遡及適用する」方向で解決された場合は、本節の再計算ロジックおよび3.2.1節のアクティビティ図の双方を見直す必要がある。
...
| 項目 | 内容 |
|---|---|
| 要件定義書の記載 | 記載なし（`[要確認 ISS-018]`：Lv1→Lv2の既存データ移行要否も含めて未確定） |
...
- 決済代行サービス等、外部サービスとの通信が将来発生する場合（ISS-003が対象範囲に含まれると確定した場合）は、アウトバウンド通信のためのNAT Gateway／Azure Firewallの追加検討が必要になる。現時点の要件定義書からは外部サービス連携の必要性が確認できないため、本図には含めていない。
...
| ISS-003 | C | 高 | 決済・レシート・返品・取消がスコープに含まれるか | 機能一覧・画面一覧・APIエンドポイント一覧のいずれにも含めていない | 3, 4, 6章 |
| ISS-004 | C | 高 | 商品マスタの新規登録・編集機能のスコープ | PRODUCTSテーブルは用意したが、登録・編集用の画面・APIは設計対象外とした | 4, 5, 6章 |
| ISS-005 | C | 高 | 値引き・消費税率の変更手段 | DISCOUNTS／TAX_RATESテーブルは用意したが、変更用の画面・APIは設計対象外とした | 5, 6章 |
| ISS-006 | A | 高 | 会員ID入力前にスキャンした値引き対象商品への適用タイミング | アクティビティ図では「登録時点で会員ID入力済み」の場合のみ値引きを適用する分岐とし、入力前スキャン分への遡及適用は設計していない。7.4.2(3)節のサーバー側金額再計算も、この登録時点の判定結果を尊重する前提であることを明記した（遡及適用ではない） | 3.2.1, 7.4.2(3)節 |
| ISS-007 | B | 高 | 複数値引きの併用可否・優先順位 | DISCOUNTSテーブルは1商品1値引きの単純な構造とし、併用・優先順位ロジックは設計していない | 5章 |
| ISS-008 | A | 高 | 存在しない会員IDが入力された場合の挙動 | 会員照会APIのレスポンス仕様として「未定」のまま一覧に記載した | 6章 |
| ISS-009 | B | 中 | バーコードスキャン失敗時の挙動 | アクティビティ図では商品マスタ未登録エラー（確定要件）のみ分岐に含め、スキャン失敗自体は注記に留めた | 3.2.1節 |
| ISS-010 | C | 中 | ログアウト機能のスコープ | ログアウトAPIを一覧には含めたが、要確認タグ付きとした | 6章 |
| ISS-012 | C | 高 | 複数税率（軽減税率）対応のスコープ | TAX_RATESテーブルは単一税率を前提とした構造とした | 5章 |
...
| ISS-020 | C | 高 | 在庫管理機能のスコープ | 在庫関連のテーブル・画面・APIは設計していない | 5, 4, 6章 |
| ISS-022 | C | 高 | 会員登録機能のスコープ | MEMBERSテーブルは「登録済みであることが前提」とし、登録・編集用の画面・APIは設計対象外とした | 5, 4, 6章 |
| ISS-024 | A | 高 | 商品登録の2段階UIフロー（検索→一時表示→追加）の継続要否 | SC-02の項目定義（名称／単価表示エリア、追加ボタン）に要確認タグを付けたまま画面項目として残した | 4.2.2節 |
| ISS-025 | B | 中 | 商品追加・購入確定後のクリア挙動の要否 | アクティビティ図・画面項目定義のいずれも、確定後の遷移先を明示せず注記に留めた | 3.2.2, 4.2.2節 |
| ISS-026 | A | 中 | 「お客様ID読み込みボタン」操作の要否 | SC-02の項目定義に要確認タグを付けて残した | 4.2.2節 |
| ISS-028 | A | 高 | 購入確定時の税表示形式の矛盾（Lv1内部） | Lv2の記述（税込・税抜両方表示）を優先する設計としたが、表示形態（ポップアップかインラインか）は未確定のまま残した | 4.2.2, 6章 |
| ISS-029 | B | 中 | 手入力の「確定」操作トリガーが未定義 | 商品コード手入力欄の確定操作を要確認タグ付きの項目として残した | 4.2.2節 |
| ISS-030 | B | 中 | 商品コードの規格（JAN／EAN／独自コード等）が未定義 | product_codeのデータ型をVARCHAR(20)と暫定的に設定した。規格確定後、桁数・文字種の見直しが必要 | 5章 |

ISS-011（複数レジ端末の同時稼働）、ISS-013（レジ業務全体の役割定義）、ISS-014（性能数値目標）、ISS-015（可用性要件）、ISS-017（運用保守要件）、ISS-018（データ移行要否）、ISS-021（生鮮食品対応）、ISS-023（REQ-005の記述矛盾）は、本書では特段の暫定処理を行わず、要件定義書の記載どおり「未定」として扱った（該当する章では設計を行っていない、または言及していない）。
...
| D-ISS-02 | 購入リスト（確定前の一時データ、DR-002相当）をフロントエンド・バックエンドのどちらが保持するか、CON-006の役割分担をどう具体化するか | 5.3, 6章 |
...
| D-ISS-07 | DISCOUNTSテーブルの「1商品1値引き」という単純化が、ISS-005（変更手段）・ISS-007（併用可否）の確認結果次第で見直しになる可能性 | 5.2節 |
| D-ISS-08 | リフレッシュトークンの発行記録・失効管理用テーブル（REFRESH_TOKENS等）を5章のデータ設計に追加するかどうか、追加する場合のテーブル構造 | 7.4.1節 |
| D-ISS-09 | レジ担当者の権限区分（ロール分け）を設ける必要があるか。現行設計は「有効なJWT＝全レジ機能を利用可」という単純な二値モデル | 7.4.1節 |
...
これらの検討課題は、要件定義書のISS一覧と同様に、発注者・開発チームでの確認・決定を経るまでは未確定のままとする。本書自体も、これらの解消に伴って改訂されることを前提とした「第1版（ドラフト）」である。
